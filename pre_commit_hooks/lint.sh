...
function lint_template_fixme() {
	...
	grep --quiet --recursive --exclude-dir node_modules --exclude ./doc/checklist.md --exclude ./bin/lint.sh TEMPLATEFIXME ${repo_dir}
	if [[ $? -eq 0 ]] && ! $is_template_repo; then
...
function lint_python_source() {
	...
	#bin/reformat-imports.sh --diff "${files_py[@]}"

	notice "Linting python source"
	# we have to run --check-only to know if there were problems
	# because --diff won't exit with an error if it finds problems
	if ! bin/reformat-imports.sh --check-only "${files_py[@]}" | { egrep -v '^Note: Skipped ' || true ; } ; then
		bin/reformat-imports.sh --diff "${files_py[@]}" | { egrep -v '^Note: Skipped ' || true ; }
		err "python imports are not correctly sorted"
	fi
}
...
function lint_js() {
	...
	then
		if [[ $( $FIND . -name .nvmrc ! -path \*node_modules\* -print -quit ) = "" ]] ; then
...
function lint_sh() {
	...
	if $files_all ; then
		IFS=$'\n'
			files_exec=( $( $FIND . -type f ! -path \*node_modules\* ! -path ./ci-cache\* \( -executable -or -name \*.inc \) ! -name \*.py -print ) )
		unset IFS
	fi
...
function lint_fixtures() {
	...
	for fixture in init groups dev_users dev ; do
		if [[ $( $FIND "$project_dir" -name $fixture.json -size $min_size -print -quit ) = "" ]] ; then
...
function lint_doc() {
	...
	if ! $is_template_repo && [[ $( $FIND "$project_dir" -name favicon.ico -print -quit ) = "" ]] ; then
...
function lint_tests() {
	...
	if [[ ! $is_template_repo ]] && grep lint-template- .gitlab-ci.yml --quiet ; then
		error "lint-template-* tests should be removed from .gitlab-ci.yml"
	fi
}
